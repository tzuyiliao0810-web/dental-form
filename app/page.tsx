      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-8 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[['clinic','診所'],['doctor','醫師'],['patient','患者']].map(([k,l]) => (
              <div key={k}>
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {[['implantBrand','植體廠牌'],['implantSystem','植體系統'],['implantSize','植體尺寸']].map(([k,l]) => (
              <div key={k}>
                <label className="text-sm text-gray-600">{l}</label>
                <input value={form[k]} onChange={e => set(k, e.target.value)}
                  className="w-full border rounded-lg px-3 py-2 mt-1" />
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[['screw','Screw'],['analog','Analog'],['transfer','Transfer'],['abutment','Abutment'],['scanBodies','Scan Bodies'],['tray','Tray']].map(([k,l]) => (
